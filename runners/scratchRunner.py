...
class scratchRunner(baseRunner):
    ...
    def __init__(self, **kwargs):
        ...
        self.initial_artifact_name = f"initial-{entity}-{project}-" \
                                     f"{self.config.arch}-{self.config.dataset}-{self.config.run_id}"
    ...
    def find_existing_model(self):
        ...
        outputStr = f"Found {self.initial_artifact_name} with seed {seed}"\
            if self.artifact is not None else "Nothing found."
